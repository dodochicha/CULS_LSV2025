...
void AIGMan::rewrite(bool fUseZeros, bool fGPUDeduplicate, bool fOptimizeLevel) {
    if (!aigCreated) {
    ...
    if(verbose>=1) {
        ...
        if (fOptimizeLevel)
            printf("rewrite: optimize level activated!\n");
    }
...
clock_t startAlgTime = clock();
    rwMan.Rewrite(fUseZeros, true, fOptimizeLevel);
